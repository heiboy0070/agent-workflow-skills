...
Opening a PR is the LAST step, not a verification step. A change reaches "ready to PR" only after the **risk-tiered PR gate** from rigorous-delivery has run against the latest commit AND you've confirmed it's mergeable: **no open P0/P1**, P2/P3 triaged with the user. High-risk changes require the current agent to complete the sequential **4b-full** matrix; low/medium-risk changes use one combined impact pass plus focused tests. Do not spawn Task/Agent/subagent reviewers unless the user explicitly requested delegation in the current task. This is not a whole-repository audit: inspect only the current diff and genuinely coupled flows. Reuse an existing gate/evidence ledger when the latest reviewed diff is unchanged; never rerun it solely because PR preparation started.
...
4. **Pre-PR cleanliness gate.** Before pushing, run `git diff <base>..HEAD --name-only` and confirm it contains ONLY intended files. Catch accidentally-committed scratch/untracked files (the `git add -A` trap — prefer explicit `git add <paths>`), leftover `<<<<<<<` conflict markers, and that build/tests are green.
   Also validate `head` against the `rigorous-feature-delivery` branch-name contract before presenting the PR: `<group>/<english-kebab-case-description>`, ASCII English only, with no username/owner prefix, tracker ID, issue number, Chinese, spaces, or underscores. A nonconforming branch MUST be renamed before push; user approval of the PR does not waive branch naming unless the user explicitly overrides the naming rule itself.
   Run the `rigorous-feature-delivery` workflow-artifact validator against `<base>..HEAD`. Agent-generated plan/spec/design/progress/tracker/evidence/handoff Markdown MUST NOT enter the PR. Allow a Markdown file only when the user requested that exact versioned deliverable or the repository explicitly requires it; state the exception in the PR-ready handoff.
...
2a. **Commit structure gate:** run `git log --oneline <base>..HEAD` and show:
   ...
   - whether each commit matches one of: DB/schema, core service logic, API/WS contract, tests, explicitly required product docs, review-fix
...
3. **Cleanliness gate** (Iron rule 4): `git diff <base>..HEAD --name-only` only-intended; no markers; `build` + `test` green.
   - Validate the current branch name before showing the PR-ready block. Do not present a generated tracker branch as `head` and defer renaming until after confirmation.
   - Reject workflow-only Markdown from the diff before showing the PR-ready block. Do not rationalize that it is acceptable because it was generated by a required planning skill.
...
## Red flags — STOP
- About to suggest "要不要建 PR / shall I open a PR / 可以提 PR 了" when the latest commit has not passed the matching risk-tiered PR gate → STOP, run the required impact-radius review first and confirm no open P0/P1.
