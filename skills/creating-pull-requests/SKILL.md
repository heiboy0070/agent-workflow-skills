...
Opening a PR is the LAST step, not a verification step. A change reaches "ready to PR" only after a **multi-dimensional impact-radius review** has run (rigorous-delivery **4b-full** panel — security / data-lifecycle / frontend-robustness / integration-regression as applicable, each auditing the current diff and high-coupling upstream/downstream flows in parallel, consolidated into one severity-ranked table) AND you've confirmed it's mergeable: **no open P0/P1**, P2/P3 triaged with the user. This is not a whole-repository audit: for a payment diff, review payment entry points, course/order state handoff, success/failure/retry/refund/subscription lifecycle as applicable; do not review unrelated login or student-profile areas unless the diff calls them or changes their contract. A standard single reviewer + red-team pass is **NOT** enough to even *prompt* the user to create a PR. If the impact-radius full review hasn't run against the latest commit, STOP — run it first (covering both frontend AND backend when the impacted change spans both repos); only then tell the user the work is PR-ready. If the same latest commit already passed `4b-full` and no code changed after that review, do NOT repeat it; cite the existing review evidence. Asking "要不要建 PR? / shall I open a PR?" off the standard pass alone is a violation of this gate.
...
## Process
0. **Full-review gate** (see **Gate** above): confirm the latest commit passed the 4b-full multi-dimensional impact-radius review — frontend AND backend if the impacted diff spans both — with no open P0/P1, and the consolidated table was already shown to the user. If it already ran on the same latest commit, cite it and continue; if it hasn't run or commits changed afterward, STOP and run the impact-radius full review before touching the PR flow.
...
## Red flags — STOP
- About to suggest "要不要建 PR / shall I open a PR / 可以提 PR 了" when only the standard reviewer + red-team ran (no multi-dimensional impact-radius review for the changed surface and high-coupling flows) → STOP, run 4b-full first and confirm no open P0/P1.
