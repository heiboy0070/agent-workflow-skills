...
Opening a PR is the LAST step, not a verification step. A change reaches "ready to PR" only after the **risk-tiered PR gate** from rigorous-delivery has run against the latest commit AND you've confirmed it's mergeable: **no open P0/P1**, P2/P3 triaged with the user. High-risk changes (payments/billing/money, auth/authorization, durable state machines, concurrency, external webhooks/queues, migrations, privacy/security contracts) require a **multi-dimensional impact-radius review** (rigorous-delivery **4b-full** panel — security / data-lifecycle / frontend-robustness / integration-regression as applicable, each auditing the current diff and high-coupling upstream/downstream flows in parallel, consolidated into one severity-ranked table). Low/medium-risk changes may use one comprehensive impact-radius reviewer plus focused tests unless the user asks for the full panel. This is not a whole-repository audit: for a payment diff, review payment entry points, course/order state handoff, success/failure/retry/refund/subscription lifecycle as applicable; do not review unrelated login or student-profile areas unless the diff calls them or changes their contract. If the matching risk-tiered gate hasn't run against the latest commit, STOP — run it first; only then tell the user the work is PR-ready. If the same latest commit already passed the matching gate and no code changed after that review, do NOT repeat it; cite the existing review evidence.
