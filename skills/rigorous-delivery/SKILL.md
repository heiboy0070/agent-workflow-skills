...
9. **Issue-complete + pushed requires impact-radius review.** If you believe the issue is complete and the branch has been pushed, run the `4b-full` multi-dimensional review before saying the issue is done or asking whether to create a PR. This is not a whole-repository audit: scope it to the current diff and its high-coupling upstream/downstream flows. A standard normal reviewer + red-team pass is necessary but not sufficient for PR readiness.
...
**4b. Independent + adversarial review via subagent (Iron rule 7) — before claiming done.**
...
- **Red-team subagent ("扮坏人"):** explicitly tasked to BREAK the change — hostile/boundary inputs, auth bypass, concurrent/duplicate ops, empty/null, malformed/oversized payloads, contract violations, injection. Goal = surface a failure or hole. Also drive realistic load (open the list / detail view the change powers, with a representative number of rows) and watch the Network panel / query log for a request storm or N+1. **For any read-modify-write / JSONB-array / counter / upsert write, actually FIRE N concurrent real requests and assert the final state keeps all N** (Iron rule 8) — a lost/overwritten/orphaned record is a P1, not a footnote. Also diff the **create/upload response vs the read response** for the same resource — any field/flag mismatch is a real bug.
- Fix every CONFIRMED finding → re-verify the fix via real API. Skipping the red-team because smoke/tests passed is forbidden.

**4b-full. Multi-dimensional impact-radius review panel — REQUIRED (a) when the user asks for a thorough / 全方面 / comprehensive review, AND (b) by default as the GATE before you prompt the user to create a PR for any substantial change (run it even if not asked; cover frontend AND backend when the change spans both repos).** Do NOT just re-run the same reviewer that only re-checks the reported issue — that's "只盯局部". Also do NOT turn this into a whole-repository audit. Define the review radius from the actual diff:
...
Fan out a PANEL of independent reviewers, **one per dimension**, each auditing the whole impacted feature surface (not unrelated modules) from its own angle, IN PARALLEL, then consolidate. Cover (add/drop dimensions to fit the change):
...
**4c. Full / pressure verification.** The complete edge-case battery (Iron rule 4) + every case the red-team surfaced + the test suite (e.g. `go test ./...`, all pass) + frontend field check against a real response (Iron rule 5). All via real API, with raw evidence pasted (Iron rule 6). **Each edge case gets its OWN pasted artifact** — reusing the 4a happy-path as edge-case evidence is forbidden. Each behavior-changing slice/phase gets its own 4a–4c (incl. red-team); "same pattern as a prior slice" is not an exemption.

### 4d. Loop until PR-ready — don't stop mid-way to ask "没问题吧?"
A substantial task is NOT done when the first round of fixes lands. RUN THE LOOP autonomously: smoke → 4b-full panel → fix every P0/P1/P2 → **re-run reviewer + red-team on the FIX itself** (a fix can introduce a new hole or a symmetric miss — e.g. patching `selectDate` but forgetting `navigateWeek`) → repeat until a fresh pass surfaces nothing new AND there are no open P0/P1/P2. Do ALL the non-decision work yourself inside this loop — do not bounce back to the user after each round with "确定没问题吗? / 都 OK 了吗?", which just offloads verification onto them. The ONLY reason to pause is a genuine **human decision** (product semantics, scope trade-off, a deliberate defer of a real issue); **batch those and ask in ONE message at the end** (do the non-decision items first, leave only the decisions). Stop and hand off for acceptance only when the work is truly PR-ready: 4b-full gate passed, fixes re-verified (headed browser for UI), evidence pasted, follow-ups triaged. "I'll ask the user if it's fine" is not a substitute for looping it to clean.
...
- [ ] red-team subagent: invocation/agent id + each finding with disposition (fixed+re-verify / rejected+artifact) + final clean re-run's agent id + its pasted no-findings output
...
- [ ] before suggesting a PR: 4b-full multi-dimensional impact-radius review ran (front+back when the impacted diff spans both) and was consolidated into one severity table for the user, with no open P0/P1 — never prompt "建 PR 吗?" off the standard reviewer+red-team pass alone
...
- Treating the red-team ("扮坏人") pass as optional because the smoke test passed or the suite is green.
...
- About to ask the user "建 PR 吗? / 可以提 PR 了" for a substantial change when only the standard reviewer + red-team ran — the multi-dimensional impact-radius review (4b-full, front+back when impacted) is the GATE before suggesting a PR; run it and show the consolidated table first.
