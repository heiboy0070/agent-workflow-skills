...
**4b-full. Multi-dimensional review panel — REQUIRED (a) when the user asks for a thorough / 全方面 / comprehensive review, AND (b) by default as the GATE before you prompt the user to create a PR for any substantial change (run it even if not asked; cover frontend AND backend when the change spans both repos).** Do NOT just re-run the same reviewer that only re-checks the reported issue — that's "只盯局部". Fan out a PANEL of independent reviewers, **one per dimension**, each auditing the WHOLE feature (not only the latest fix) from its own angle, IN PARALLEL, then consolidate. Cover (add/drop dimensions to fit the change):
...
Each panel reviewer hits the REAL API with named evidence and returns P0–P3 + file:line; keep the standard normal-reviewer + red-team too. Then **YOU consolidate ALL findings into ONE severity-ranked table** and present scope choices to the user (fix now vs separate ticket) — never silently drop or unilaterally decide. Goal: review the whole surface multi-angle so a later **external** reviewer finds nothing you didn't.
...
- [ ] before suggesting a PR: 4b-full multi-dimensional review ran (front+back when the change spans both) and was consolidated into one severity table for the user, with no open P0/P1 — never prompt "建 PR 吗?" off the standard reviewer+red-team pass alone
